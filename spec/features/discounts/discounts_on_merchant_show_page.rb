...
RSpec.describe "As a user when I visit a merchant's show page" do
  before(:each) do
    ...
    @discount1 = @meg.discounts.create!(name: "Flash Sale: 10% off 5 items", percentage: 10, item_amount: 5)
  ...
  it "I see a section of that page that lists all that merchant's discounts" do
    visit "/merchants/#{@meg.id}"
    within "#merchant-discounts" do
      expect(page).to have_content("We have specials on offer for bulk orders:") 
    ...
    within "#merchant-discounts" do
      expect(page).to have_content("We currently have no specials on offer") 
    end
  end
end