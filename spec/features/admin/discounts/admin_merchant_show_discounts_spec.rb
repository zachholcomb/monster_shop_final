...
RSpec.describe 'As an admin when I visit a merchants show page' do
  ...
  it 'I see their discounts listed on their profile page' do
    visit "/admin/merchants/#{@meg.id}"
    expect(page).to have_content(@discount1.name) 
    expect(page).to have_content(@discount2.name) 
  end

  it "I can click on a discount link and am taken to that discounts show page" do
    visit "/admin/merchants/#{@meg.id}"
    click_link @discount1.name
    expect(page).to have_current_path("/admin/merchants/#{@meg.id}/discounts/#{@discount1.id}") 
    expect(page).to have_content(@discount1.name)
  end
end